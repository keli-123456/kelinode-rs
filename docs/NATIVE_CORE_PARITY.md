...
| Trojan | Partial | TCP, TCP+TLS, UDP ASSOCIATE over stream, and route outbounds for WS/HTTPUpgrade/H2/gRPC/TLS; XHTTP/splithttp stream-one route outbounds render as native H2; old-QUIC route outbounds render with `none`/`aes-128-gcm`/`chacha20-poly1305` packet security when `header.type` is none | Trojan inbound WS/TLS WS are currently capability-gated and rejected for default native rendering; KCP, QUIC packet header obfuscation, XHTTP packet-up/stream-up/H3 |
