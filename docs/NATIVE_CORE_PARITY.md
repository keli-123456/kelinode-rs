...
| VLESS | Partial | TCP, UDP command, WS, HTTPUpgrade, H2, gRPC, TLS, Vision, REALITY; XHTTP/splithttp stream-one route outbounds render as native H2; plain old-QUIC route outbounds render when `security/key/header` are all none/empty | XUDP/Mux, KCP, QUIC packet auth/header wrapping, XHTTP packet-up/stream-up/H3, unsupported flow, REALITY on non-TCP |
| VMess | Partial | TCP, UDP command, WS, HTTPUpgrade, H2, gRPC, TLS, legacy alterId route outbound rendering; XHTTP/splithttp stream-one route outbounds render as native H2; plain old-QUIC route outbounds render when `security/key/header` are all none/empty | Legacy alterId inbound, KCP, QUIC packet auth/header wrapping, XHTTP packet-up/stream-up/H3 |
| Trojan | Partial | TCP, UDP ASSOCIATE over stream, WS, HTTPUpgrade, H2, gRPC, TLS; XHTTP/splithttp stream-one route outbounds render as native H2; plain old-QUIC route outbounds render when `security/key/header` are all none/empty | KCP, QUIC packet auth/header wrapping, XHTTP packet-up/stream-up/H3 |
