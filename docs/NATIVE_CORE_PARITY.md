...
| Custom outbound route | Partial | Freedom, SOCKS5, HTTP, Shadowsocks, Trojan, and VLESS TCP outbound tags render into native core. Freedom supports direct address/port redirects; SOCKS5/HTTP support TCP proxy tunnels with username/password. SOCKS5 also supports UDP route outbounds through native UDP ASSOCIATE. Shadowsocks supports native AEAD TCP/UDP route outbounds. Trojan and VLESS support TCP, TLS TCP, WS TCP, and HTTPUpgrade TCP route outbounds. HTTP UDP, Trojan UDP, VLESS flow/UDP, VMess outbounds, and gRPC outbounds remain Xray-only. |
